 ...
 */
public class ModelSpriteProvider extends GenericTextureGenerator implements RuntimeResourceProvider {
  ...
  public void addToDynamicPack(DynamicResourceRegistrar registrar) {
    ...
    for (SlimeType slime : SlimeType.values()) {
      String name = slime.getSerializedName();
      ResourceLocation congealed = getResource("block/slime/storage/congealed_" + name);
      stackSprites(registrar, getResource("block/wood/enderbark/roots/" + name), rootsSide, congealed);
      stackSprites(registrar, getResource("block/wood/enderbark/roots/" + name + "_top"), rootsTop, congealed);
    }

    ISpriteTransformer stoneColor = new RecolorSpriteTransformer(GreyToColorMapping.builderFromBlack().addARGB(63, 0xFF181818).addARGB(102, 0xFF494949).addARGB(140, 0xFF5A5A5A).addARGB(178, 0xFF787777).addARGB(216, 0xFF95918D).addARGB(255, 0xFFB3B1AF).build());
    transformSprite(registrar, getResource("item/tool/parts/plating_helmet"),     getResource("item/tool/armor/plate/helmet/plating"), new OffsettingSpriteTransformer(stoneColor, 0, 2));
    transformSprite(registrar, getResource("item/tool/parts/plating_chestplate"), getResource("item/tool/armor/plate/chestplate/plating"), stoneColor);
    transformSprite(registrar, getResource("item/tool/parts/plating_leggings"),   getResource("item/tool/armor/plate/leggings/plating"), new OffsettingSpriteTransformer(stoneColor, 0, 1));
    transformSprite(registrar, getResource("item/tool/parts/plating_boots"),      getResource("item/tool/armor/plate/boots/plating"), stoneColor);
  ...
  protected void transformSprite(DynamicResourceRegistrar registrar, ResourceLocation output, ResourceLocation input, ISpriteTransformer transformer) {
    try {
      NativeImage original = spriteReader.read(input);
      NativeImage generated = transformer.transformCopy(original, true);
      saveImage(registrar, output, generated);
      JsonObject meta = transformer.animationMeta(original);
      if (meta != null) {
        saveMetadata(registrar, output, meta);
      }
    } catch (IOException e) {
      throw new IllegalStateException("Error transforming sprite " + input + " into " + output, e);
    }
  }
  ...
  protected void stackSprites(DynamicResourceRegistrar registrar, ResourceLocation output, ResourceLocation... inputs) {
    List<NativeImage> sprites = Arrays.stream(inputs).map(path -> {
      try {
        return spriteReader.read(path);
      } catch (IOException e) {
    ...
    if (metaLocation != null) {
      try {
        saveMetadata(registrar, output, spriteReader.readMetadata(metaLocation));
      } catch (IOException e) {
        throw new IllegalStateException("Failed to save sprite metadata", e);
      }
    }
  }
