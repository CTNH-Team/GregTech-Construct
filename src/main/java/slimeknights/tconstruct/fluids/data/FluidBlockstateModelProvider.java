...
public class FluidBlockstateModelProvider extends GenericDataProvider implements RuntimeResourceProvider {
  ...
  public CompletableFuture<?> run(CachedOutput cache) {
    JsonObject blockstate = makeBlockstate();
    return allOf(
  ...
  public void addToDynamicPack(DynamicResourceRegistrar registrar) {
    ...
    BuiltInRegistries.BLOCK.entrySet().stream()
      .filter(entry -> entry.getKey().location().getNamespace().equals(modId) && entry.getValue() instanceof LiquidBlock)
      .forEach(entry -> registrar.addBlockState(entry.getKey().location(), blockstate));
  }

  private static JsonObject makeBlockstate() {
